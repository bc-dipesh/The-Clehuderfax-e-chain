...
(
    ACCESS_DETAIL_ID     NUMBER(6, 0),
    ACCESSED_USER_ID     NUMBER(6, 0),
    ACCESSED_BY_ADMIN_ID NUMBER(6, 0),
    ACTION               VARCHAR2(50),
    ACCESSED_AT          TIMESTAMP(0),

    CONSTRAINT PK_ACCESS_DETAIL PRIMARY KEY (ACCESS_DETAIL_ID),
    CONSTRAINT FK_ACCESS_DETAIL_1 FOREIGN KEY (ACCESSED_USER_ID) REFERENCES USERS (USER_ID) ON DELETE CASCADE,
    CONSTRAINT FK_ACCESS_DETAIL_2 FOREIGN KEY (ACCESSED_BY_ADMIN_ID) REFERENCES ADMINS (ADMIN_ID) ON DELETE CASCADE
);
...
DROP TRIGGER INCREASE_AVAILABLE_ORDER;
DROP TRIGGER DECREASE_AVAILABLE_ORDER;
...
CREATE OR REPLACE TRIGGER INSERT_ACCESS_DETAILS_ID
    BEFORE INSERT
    ON ACCESS_DETAILS
    FOR EACH ROW
    WHEN (NEW.ACCESS_DETAIL_ID IS NULL)
BEGIN
    :NEW.ACCESS_DETAIL_ID := ACCESS_DETAILS_SEQ.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER DECREASE_AVAILABLE_ORDER
    AFTER INSERT
    ON INVOICES
    FOR EACH ROW
BEGIN
    UPDATE COLLECTION_SLOTS CS
    SET CS.AVAILABLE_ORDER = CS.AVAILABLE_ORDER - 1
    WHERE CS.COLLECTION_SLOT_ID = :NEW.COLLECTION_SLOT_ID;
END;
/

CREATE OR REPLACE TRIGGER INCREASE_AVAILABLE_ORDER
    AFTER UPDATE
    ON INVOICES
    FOR EACH ROW
BEGIN
